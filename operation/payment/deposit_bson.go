...
type DepositFactBSONUnmarshaler struct {
	...
	TransferLimit string   `bson:"transfer_limit"`
...
func (fact *DepositFact) DecodeBSON(b []byte, enc *bsonenc.Encoder) error {
	...
	err = amount.DecodeBSON(uf.Amount, enc)
	if err != nil {
		return common.DecorateError(err, common.ErrDecodeBson, *fact)
	}
	fact.amount = amount
	...
	if err := fact.unpack(
		enc, uf.Sender, uf.Contract, uf.TransferLimit, uf.StartTime, uf.EndTime, uf.Duration,
	); err != nil {
		return common.DecorateError(err, common.ErrDecodeBson, *fact)
	}
